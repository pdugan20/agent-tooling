...
VERBS. Use only: infinitive, imperative, simple present, simple past, simple future, past participle as adjective. No present perfect ("has completed" → "completed"). No "-ing" verb forms ("making it easy" → new sentence). Active voice; passive only in descriptions when the agent is unknown. Approved modals: can, will, must. Banned: should, would. For "should": write "must" if required, delete if optional.

HEDGES ARE CONTENT. Keep "may", "might", or "could" when they carry real uncertainty about a fact. "The request may have failed" and "the request failed" are different claims, and the shorter one is not a simplification — it is a fabrication. Never promote a hedge to a fact to satisfy a length rule. Delete a hedge only when it is padding ("this may potentially help to improve"), never when it states confidence. The same applies in reverse: never invent uncertainty the source did not express.
